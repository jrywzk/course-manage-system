...
def main():
    ...
    with open(OUTPUT, "w", encoding="utf-8") as f:
        ...
        for i in range(GEN["teacher"]):
            rows.append((f"tch{i+5:05d}", "123456", "teacher", 1, "2024-01-01 00:00:00"))
            teacher_uids.append(uid); uid += 1
        # 学生
        for i in range(GEN["student"]):
            rows.append((f"stu{i+9:05d}", "123456", "student", 1, "2024-09-01 00:00:00"))
        ...
        for i, u in enumerate(admin_uids):
            rows.append((u, f"ADM{2025:04d}{i+3:04d}", chinese_name(),
        ...
        for i, u in enumerate(teacher_uids):
            ...
            dept_code = dc[(i + 4) % len(dc)]  # 分散到各院系
            rows.append((
                u, all_dept_code_id[dept_code],
                f"T{2025:04d}{i+5:04d}",
                chinese_name(),
                random.choice(TITLES),
                phone(), email("tch", i+5), 1
            ))
        ...
        # ────────────────────────────────────────────────
        # 9. t_course_section (从10 → 6000, 新增5990条)
        # ────────────────────────────────────────────────
        print("[9/11] t_course_section ...")
        # 每门课在若干学期各开1-3个教学班
        rows = []
        goal = GEN["course_section"]
        sem_pool = SEMESTERS
        attempts = 0
        while len(rows) < goal and attempts < 10000:
            attempts += 1
            cid = random.choice(all_course_ids)
            sem = random.choice(sem_pool)
            # 每个教学班 1-3 个 section
        ...
        # 按学期组织教学班 & 建立 section→semester 映射
        sections_by_sem = {}
        ...
        for enr_id in scored_eids:
            ...
            g_by = f"教师{random.randint(1, TARGET['teacher']):05d}"
    ...
    actual = {
        "department": 20, "classroom": 500, "major": 80,
        "user": 12548, "admin": 20, "teacher": 500,
        "student": 12028, "course": 108,
        "course_section": 6000, "enrollment": 60022, "score": 55014,
    }
